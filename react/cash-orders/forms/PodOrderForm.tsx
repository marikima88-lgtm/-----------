...
import { BRANCHES, CURRENT_ACCOUNTANT, CURRENT_CASHIER, DESKS, PODOTCHET_PERSONS } from '../mockData';
...
interface PodOrderFormProps {
  direction: Direction;
  defaultValues?: Partial<PodFormValues>;
  editable: boolean;
  showCancel: boolean;
  onSaveDraft: (values: PodFormValues) => void;
  onSend: (values: PodFormValues) => void;
  onRequestCancel: () => void;
}

export function PodOrderForm({
  direction,
  defaultValues,
  editable,
  showCancel,
        ...
        <Grid container spacing={2} sx={{ mb: 2 }}>
          ...
          <Grid item xs={12} sm={6}>
            <Controller
              name="person"
              control={control}
              rules={{ required: 'Выберите подотчётное лицо' }}
              render={({ field, fieldState }) => (
                <TextField
                  {...field}
                  select
                  label="Подотчётное лицо"
                  required
                  fullWidth
                  disabled={!editable}
                ...
                >
                  <MenuItem value="">— Выбрать из справочника —</MenuItem>
                  {PODOTCHET_PERSONS.map((p) => (
        ...
        <Controller
          ...
          render={({ field, fieldState }) => (
            <TextField
              ...
              label="Сумма"
              required
              sx={{ maxWidth: 340, display: 'block', mb: 2 }}
              fullWidth
              disabled={!editable}
